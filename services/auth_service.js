...
class AuthService {
    login = async (name, password) => {
        try {
            ...
            const accessToken = await jwtService.generateToken({sub: user.id, name: user.name, role: user.role}, '15m', 'access');
            const refreshToken = await jwtService.generateToken({ sub: user.id }, '7d', 'refresh');
            return {
